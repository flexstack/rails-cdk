...
const projectName = process.env.PROJECT_NAME ?? "rails-cdk";
...
const railsStack = new cdk.Stack(app, "Rails", {
  stackName: `Rails-${projectName}-${environment}`,
  env,
});

const taskDefinition = new TaskDefinition(railsStack, "TaskDefinition", {
  family: `${projectName}-rails-${environment}`,
  cpu: "256",
  memoryMiB: "512",
  runtimePlatform: {
...
const container = taskDefinition.addContainer("RailsContainer", {
  image: ContainerImage.fromAsset("../", {
    exclude: [".git", ".readme", "node_modules", "infra", "tmp"],
    platform: platform,
    buildArgs: {
      TARGETOS: "linux",
  ...
  linuxParameters: new LinuxParameters(railsStack, "LinuxParameters", {
    initProcessEnabled: true,
  }),
  secrets: {
    RAILS_MASTER_KEY: Secret.fromSsmParameter(
      StringParameter.fromSecureStringParameterAttributes(
        railsStack,
        "RailsMasterKey",
        {
          parameterName: `/${projectName}/${environment}/RAILS_MASTER_KEY`,
        }
      )
    ),
  },
...
const serviceSg = new SecurityGroup(railsStack, "ServiceSecurityGroup", {
  vpc,
  allowAllIpv6Outbound: true,
  allowAllOutbound: true,
});

const fargateService = new FargateService(railsStack, "FargateService", {
  cluster,
  taskDefinition,
...
const apiGw = new HttpApi(railsStack, "HttpApi", {
  apiName: `${projectName}-${environment}`,
  disableExecuteApiEndpoint: false, // Set to true if using a custom domain
});

const vpcLinkSg = new SecurityGroup(railsStack, "VpcLinkSecurityGroup", {
  vpc,
  allowAllIpv6Outbound: true,
  allowAllOutbound: true,
});
...
const vpcLink = new VpcLink(railsStack, "VpcLink", {
  vpc,
...
const distribution = new Distribution(railsStack, "CDN", {
  defaultBehavior: {
    ...
    compress: true,
    cachePolicy: new CachePolicy(railsStack, "CachePolicy", {
